...
final class TodayMdStore {
    ...
    func applyImport(_ archive: TodayMdArchive, mode: ImportMode) {
        performMutation(actionName: mode == .merge ? "Import Tasks" : "Replace Tasks") {
            ...
            let hydrated = archive.remappedForMerge().instantiate()
            ...
            for block in TimeBlock.allCases {
                let importedTasks = hydrated.unassignedTasks
                    .filter { $0.block == block }
                    .sorted(by: taskSort)
                let taskSortBase = nextSortOrder(for: nil, in: block)

                for (index, task) in importedTasks.enumerated() {
    ...
    private func refreshSearch() {
        ...
        do {
            persistedSearchIDs = try persistenceQueue.sync {
                try database.searchTaskIDs(matching: searchText)
            }
        } catch {
            persistedSearchIDs = []
